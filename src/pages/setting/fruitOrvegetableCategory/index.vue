...
export default {
  ...
  methods: {
    tabChange(val) {
      ...
      if(val == 1){
        this.$nextTick(()=>{
          this.$refs.veggietable && this.$refs.veggietable.init(this.nodeData);
        })
      }  else{
        this.$nextTick(()=>{
          this.$refs.tabComponent && this.$refs.tabComponent.init(this.nodeData);
        })
      }
    },
          ...
          if (res.retCode === 200) {
            ...
            if (res.retData.length && res.retData[0].children.length) {
              this.$nextTick(() => {
                ...
                this.nodeData = data;
                this.$nextTick(()=>{
                  this.$refs.veggietable && this.$refs.veggietable.init(data);
                  this.$refs.tabComponent && this.$refs.tabComponent.init(data);
                })
    ...
    treeClick({ node }) {
      ...
      if (node.data.parentId != 0) {
        ...
        this.tabValue = 1;
        this.$nextTick(()=>{
          this.$refs.veggietable && this.$refs.veggietable.init(node.data);
        })
      }
    },
    addCategory() {
...
/deep/.t-is-active .t-tree__label {
  background-color: #215bc136 !important;
}
.list-tree-content {
